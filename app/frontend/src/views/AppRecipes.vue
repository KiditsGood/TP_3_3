      ...
      <div class="main">
          <div class="main__flex">
              <div class="main__products">
                  <RecipeCard v-for="recipe in searchAndSortHandler" :recipe="recipe" :key="recipe.id"/>
              </div>
...
export default {
    ...
    computed: {
        sortedPosts() {
            return [...this.recipes].sort((product1, product2) => this.sortValue === 'По возрастанию' ? product1.price - product2.price : this.sortValue === 'По убыванию' ? product2.price - product1.price : this.products)
        },

        searchAndSortHandler() {
            return this.sortedPosts.filter(recipe => recipe.name.toLowerCase().includes(this.searchQuery.toLowerCase()))
        }
    },
