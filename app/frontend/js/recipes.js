...
function categoryPage () {
    ...
    $.ajax({
        method: 'GET',
        url: 'https://tp3-3.herokuapp.com/recipes?search=name:*' + searchValue + '*&page_number=0&page_size=4',
        dataType: 'json',
        success: paginationData => {
            ...
            paginationData.items.forEach(el => {
                ...
                el.products.forEach(recipeItem => {
                    $('#recipe').append(`
                    <div class="products__item">
                        ...
                        <img class="products__item-image" src="${recipeItem.mainPhoto}"/>
                        <p class="products__item-title">${recipeItem.name}</p>
                        <div class="products__desc">
                            <a onclick="descWatcher(this)" class="products__item-watch">Подробнее</a>
                            <p class="products__item-description">${recipeItem.description}</p>
                        </div>
                        <div class="products__item-cost--flex">
                            <p class="products__item-cost">${recipeItem.price}</p>
                ...
                $('.main__products').append(`
                        <div class="products__item">
                            ...
                            <p class="products__item-food">Кол-во ингридиентов: ${el.products.length}</p>
                            <div class="products__desc">
                                <a onclick="descWatcher(this)" class="products__item-watch">Подробнее</a>
                                <p class="products__item-description">${el.description}</p>
                            </div>
                            <div class="products__item-cost--flex">
                                <a href="#recipe" rel="modal:open" class="products__item-buy">Используемые продукты</a>
                            </div>
                        </div>
                `)
            })
...
function paginationItem(paginationNum){
    ...
    $.ajax({
        method: 'GET',
        url: 'https://tp3-3.herokuapp.com/recipes?search=name:*' + searchValue + '*&page_number=' + pageNumber + '&page_size=4',
        dataType: 'json',
        success: searchData => {
